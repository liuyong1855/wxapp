...
Page({
    ...
    onLoad: function (options) {
        const demodays_count = new Date(this.data.year, this.data.month, 0).getDate();
        let demo_days_style = new Array;
        for (let i = 1; i <= demodays_count; i++) {
            if (i <= 9 && i != 7) {
                demo_days_style.push({
                    month: 'current', day: i, color: '#fff', background: '#f00'
                });
            } else {
                demo_days_style.push({
                    month: 'current', day: i, color: '#f0f'
                });
            }
        }
        console.log(demo_days_style)
        this.setData({
            demo_days_style
        });
